...
#import "JLP2PFinancialView.h"
#import "JLInterestTableVC.h"
#import "JLPickerview.h"
#import "JLBankFinancialModel.h"
#import "JLDatePickerView.h"
#import "JLP2PfinancialModel.h"
#import "JLCheckVC.h"
#import "JLPayMoneyModel.h"
#import "JLRecommendModel.h"

@interface JLFinancialCalculatorVC ()<UIGestureRecognizerDelegate,UITextFieldDelegate>
@property (nonatomic ,strong)JLP2PFinancialView *ptwopView;//点击P2P理财显示的view
@property (nonatomic ,strong)JLBankFinancingView *bankfiView;//点击银行理财显示的view
@property (nonatomic ,copy)NSString *jilustring;//记录是在那个按钮进去的
//银行理财
@property (nonatomic ,strong)JLPickerview *pickerView;
...
//P2P理财
@property (nonatomic ,strong)JLDatePickerView *JLDatePickerView;
...
@property (nonatomic ,strong)JLP2PfinancialModel *model;
...
@implementation JLFinancialCalculatorVC
...
- (void)viewDidLoad
{
    ...
    //点击P2P理财显示的view
    _ptwopView = [[JLP2PFinancialView alloc]initWithFrame:CGRectMake(0, 80*BOScreenH/1334, BOScreenW, BOScreenH)];
    ...
    //点击银行理财显示的view
    _bankfiView = [[JLBankFinancingView alloc]initWithFrame:CGRectMake(0, 80*BOScreenH/1334, BOScreenW, BOScreenH)];
    ...
    bgView.backgroundColor = mainBarColor;
    ...
    //pickerview
    _pickerView = [[JLPickerview alloc]initWithFrame:CGRectMake(0, 0, BOScreenW, BOScreenH)];
    ...
    //JLDatePickerView
    _JLDatePickerView = [[JLDatePickerView alloc]initWithFrame:CGRectMake(0, 0, BOScreenW, BOScreenH)];
    [[UIApplication sharedApplication].keyWindow addSubview:_JLDatePickerView];
    [_JLDatePickerView.sureButtons addTarget:self action:@selector(sureButtonsClick) forControlEvents:UIControlEventTouchUpInside];//确定按钮的点击事件
    [_JLDatePickerView.cancelButtons addTarget:self action:@selector(cancelButtonsClick) forControlEvents:UIControlEventTouchUpInside];
    _JLDatePickerView.hidden = YES;
    //添加手势单击事件
    ...
    [_JLDatePickerView addGestureRecognizer:pkGess];
...
- (void)bankFinancingData
{
    ...
    [manager POST:[NSString stringWithFormat:@"%@Common/jisuanqiBank",BASEURL] parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        {
            JLBankFinancialModel *model = [JLBankFinancialModel mj_objectWithKeyValues:responseObject[@"data"]];
...
- (void)ptwopData
{
    ...
    [manager POST:[NSString stringWithFormat:@"%@Common/jisuanqiWd",BASEURL] parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if ([responseObject[@"r"] integerValue] == 1)
        {
            _model = [JLP2PfinancialModel mj_objectWithKeyValues:responseObject[@"data"]];
            ...
            for (JLPayMoneyModel *listmodel in _model.huankuan_list)
            {
...
{
    [self.view endEditing:YES];
    _JLDatePickerView.hidden = NO;
}
//创建一个PickerView日期格式器 (确定 按钮的点击事件)
- (void)sureButtonsClick
{
    NSDate *selected = [_JLDatePickerView.JLDatePickerView date];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    ...
    if ([touch.view isDescendantOfView:_pickerView.buttonView] || [touch.view isDescendantOfView:_pickerView.payPicView] || [touch.view isDescendantOfView:_JLDatePickerView.buttonViews] ||[touch.view isDescendantOfView:_JLDatePickerView.JLDatePickerView])
    {
        return NO;
    }
    return YES;
...
{
    JLInterestTableVC *interVc = [[JLInterestTableVC alloc]init];
...
- (void)lookButtonClick
{
    if (_model.huankuan_list.count > 0)
    {
        if (_ptwopView.inputsTextField.text.length>0&&_ptwopView.monthTextField.text.length>0&&_ptwopView.yearTextField.text.length>0)
        {
            JLCheckVC *chakanvc = [[JLCheckVC alloc]init];
...
- (void)theInputTatetuijianpingtai
{
    ...
    [manager POST:[NSString stringWithFormat:@"%@Common/getTuijianByApr",BASEURL] parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if ([responseObject[@"r"] integerValue] == 1)
        {
            JLRecommendModel *model = [JLRecommendModel mj_objectWithKeyValues:responseObject[@"data"]];
